...
// * Revised by 7DS'Lust
//=============================================================================
class sgGuidedWarShell extends sgWarShell;
...
simulated function Destroyed()
{
	...
	{
		W = sgNukeLauncher(Guider.FindInventoryType(class'sgNukeLauncher'));
		if ( W != None )
		{
			W.GuidedShell = None;
	...
	if ( Level.NetMode == NM_Client )
	{
		if ( (PlayerPawn(Instigator) != None) && (ViewPort(PlayerPawn(Instigator).Player) != None) )
		{
			...
			if ( bDestroyed || (Instigator.health < 0) )
			{
				...
				Destroy();
				if ( Instigator.Weapon.IsA('sgNukeLauncher') )
					sgNukeLauncher(Instigator.Weapon).bGuiding = false;
				return;
			}
			PlayerPawn(Instigator).ViewTarget = self;
			if ( Instigator.Weapon.IsA('sgNukeLauncher') )
			{
				sgNukeLauncher(Instigator.Weapon).GuidedShell = self;
				sgNukeLauncher(Instigator.Weapon).bGuiding = true;
			}
		}
		else
		{
